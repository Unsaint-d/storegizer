...
erDiagram
    ...
    CATEGORIES |o--o{ TAGS : "scope_category_id"
    ...
    TAGS {
        ...
        int scope_category_id FK "NULL = применим везде"
        int created_in_work_session_id FK
    }
    ITEM_TAGS {
        int item_id FK
...
**Область действия (scope).** Тег привязывается к поддереву категорий, где
он вообще имеет смысл — `tags.scope_category_id`. «Потайной», привязанный
к «Крепёж», применим к любому предмету внутри этого поддерева (Крепёж,
Винт, М2, М3, ...), но не к предмету из «Лекарства». Проверяется при
создании предмета: если у именованного тега уже есть scope, категория
предмета обязана быть этой категорией или её потомком — иначе вся
операция отклоняется целиком, до какой-либо записи в БД (не «создали
предмет, потом не удалось прилинковать тег»).

Новый тег, созданный **inline** через `tag_names` при создании предмета,
всегда без scope (иначе первое же случайное использование зафиксировало
бы его на слишком узкой категории — опять то же дробление, которого
теги должны избегать). Чтобы тег был по-настоящему привязан к области —
его нужно создать заранее, явно, с `scope_category_id`, через
`POST /api/session/tags` — это и есть тот самый «механизм создания тегов
и категорий из админ-панели»: обе точки создания (`/api/session/categories`,
`/api/session/tags`) не привязаны к созданию предмета, это самостоятельные
действия панели.
