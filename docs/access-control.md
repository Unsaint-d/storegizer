...
| **Войти как читатель** | Требуется какое-то действие для входа, но не обязательно личные логин/пароль. |
| **По логину и паролю** | Читатель заходит под персональными кредами, как и админ. |
...
полноценный аккаунт), или тоже персональные креды, только менее
привилегированные, чем в третьем режиме? Второй режим явно задуман как
более лёгкий, чем третий — иначе они были бы одним и тем же пунктом.

## Вход администратора

Отдельная кнопка (доступна из каталога независимо от того, что выбрано
в переключателе выше — иначе к серверу нельзя будет вообще никак
подключиться администратору, если чтение закрыто) — **всегда** по
логину/паролю, без вариантов. Учётные данные — из списка доверенных
пользователей в конфигурации сервера (админов может быть несколько).
...
**внутри** этой панели, не часть самого входа. То есть вход администратора
и открытие сессии работы — два последовательных, разных действия:

```mermaid
flowchart TD
    A[Кнопка «Войти как администратор»] --> B[Логин + пароль]
...
2. **Точная семантика «войти как читатель»** (см. выше) — общий барьер без
   личности или персональные, но менее строгие креды.
3. **Список читателей при режиме «по логину и паролю»** — отдельный список
   в конфиге (аналогично списку админов) или что-то ещё?
4. **Таймаут/логаут** сессии в браузере — как у сессии работы (10 минут
   простоя, §6) или отдельное значение.
5. Защита формы входа от подбора пароля (rate limiting) — не обсуждалось,
